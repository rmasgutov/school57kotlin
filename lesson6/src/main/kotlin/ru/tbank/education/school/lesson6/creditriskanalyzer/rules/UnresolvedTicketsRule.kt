...
class UnresolvedTicketsRule(
    ...
    override fun evaluate(client: Client): ScoringResult {
        val ticketList = ticketRepo.getTickets(client.id)

        if (ticketList.isEmpty()) {
            return ScoringResult(ruleName, PaymentRisk.LOW)
        }

        val unresolvedCount = ticketList.count { !it.resolved }
        val unresolvedRatio = unresolvedCount.toDouble() / ticketList.size
